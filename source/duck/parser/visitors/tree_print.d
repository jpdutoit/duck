...
struct TreePrint {
	...
	Node visit(MemberExpr expr) {
		writefln("%s.%s - %s", padding, expr.identifier.value, mangled(expr.exprType));
		depth++;
	...
	Node visit(LiteralExpr expr) {
		if (expr.token.type == StringLiteral)
			writefln("%s'%s' - %s", padding, expr.token.value[1..$-1], mangled(expr.exprType));
		else
			writefln("%s%s - %s", padding, expr.token.value, mangled(expr.exprType));
		return expr;
	}
	...
	Node visit(BinaryExpr expr) {
		writefln("%s%s - %s", padding, expr.operator.value, mangled(expr.exprType));
		depth++;
	...
	Node visit(CallExpr expr) {
		if (auto declExpr = cast(DeclExpr)expr.expr) {
				auto s = declExpr.identifier.value;
				writefln("%sCall %s : %s - %s", padding, s, mangled(expr.expr.exprType), mangled(expr.exprType));
		} else {
			writefln("%sCall %s - %s", padding, mangled(expr.expr.exprType), mangled(expr.exprType));
			depth++;
			expr.expr.accept(this);
			depth--;
		}
	...
	Node visit(DeclStmt stmt) {
		writefln("%sDeclStmt %s - %s", padding, stmt.identifier.value, mangled(stmt.expr.exprType));
		depth++;
	...
	Node visit(DeclExpr expr) {
		writefln("%sDeclExpr %s - %s", padding, expr.identifier.value, mangled(expr.exprType));
		//depth++;
		//expr.decl.accept(this);
		//depth--;
		return expr;
