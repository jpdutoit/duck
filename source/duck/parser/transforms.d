...
import duck.compiler.ast, duck.compiler.lexer, duck.compiler.visitors, duck.compiler.buffer;
...
class TransformVisitor : Visitor!Node {
  ...
  Node visit(StructDecl decl) {
    foreach(ref field; decl.symbols) {
      transform(field);
    }
    return decl;
  }
  ...
  Node visit(FieldDecl decl) {
    ...
    if (decl.valueExpr)
      transform(decl.valueExpr);
    return decl;
  }
  Node visit(AliasDecl decl) {
    //transform(decl.typeExpr);
    transform(decl.targetExpr);
    return decl;
  }

  Node visit(MethodDecl decl) {
    transform(decl.methodBody);
    return decl;
  }
...
class PipeSplit : TransformVisitor {
  ...
  override Node visit(PipeExpr pipe) {
    ...
    if (depth > 0) {
      ...
      if (auto decl = cast(InlineDeclExpr)(pipe.right)) {
        return new IdentifierExpr(decl.declStmt.identifier);
      }
      return pipe.right;
    }
    return pipe;
